...
dds4g <- dds4g[ rowSums(counts(dds4g)) > 5, ]
...
```{r}
coexp <- merge(res_tbla, res_tbl0, by='gene', all=TRUE, suffixes=c('_a', '_0')) 
coexp1 <- merge(res_tbl1, res_tbl2, by='gene', all=TRUE, suffixes=c('_1', '_2')) 
coexp2 <- merge(res_tbl3, res_tblag, by='gene', all=TRUE, suffixes=c('_3', '_ag'))
coexp3 <- merge(res_tbl0g, res_tbl1g, by='gene', all=TRUE, suffixes=c('_0g', '_1g'))
coexp4 <- merge(coexp3, res_tbl4g, by='gene', all=TRUE, suffixes=c('_0g', '_1g'))
coexp4 <- merge(coexp, coexp1, by ='gene', all=TRUE, no.dups = TRUE) 
coexp <- merge(coexp4, coexp2, by ='gene', all=TRUE, no.dups = TRUE)
...
df$up4[(coexp$padj_4 <0.05 & coexp$log2FoldChange_4 > 0)] <- 1

#down
df$dn0[(coexp$padj_0 <0.05 & coexp$log2FoldChange_0 < 0)] <- 1
df$dn1[(coexp$padj_1 <0.05 & coexp$log2FoldChange_1 < 0)] <- 1
df$dn2[(coexp$padj_2 <0.05 & coexp$log2FoldChange_2 < 0)] <- 1
df$dn3[(coexp$padj_3 <0.05 & coexp$log2FoldChange_3 < 0)] <- 1
df$dn4[(coexp$padj_4 <0.05 & coexp$log2FoldChange_4 < 0)] <- 1

#fill in NA as zero
df[is.na(df)] <- 0

#make combination matrix, intersect mode allows overlap between categories
cm <- make_comb_mat(df,
                    mode = c("intersect"))
cm <- cm[, -c(24:32)]

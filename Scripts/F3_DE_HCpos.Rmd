...
glut0 <- subset(glut0, idents = c("homecage","positive"))
glut1 <- subset(glut1, idents = c("homecage","positive"))
glut2 <- subset(glut2, idents = c("homecage","positive"))
glut3 <- subset(glut3, idents = c("homecage","positive"))

#Collapse into 1 cluster to run 'all glut' together
all.glut <- FindClusters(glut.subset2, resolution = 0)
Idents(all.glut) <- 'group'
all.glut <- subset(all.glut, idents = c("homecage","positive"))
...
gaba0 <- subset(gaba0, idents = c("homecage","positive"))
gaba1 <- subset(gaba1, idents = c("homecage","positive"))
gaba4 <- subset(gaba4, idents = c("homecage","positive"))

all.gaba <- FindClusters(gaba.subset2, resolution = 0)
Idents(all.gaba) <- 'group'
all.gaba <- subset(all.gaba, idents = c("homecage","positive"))
...
metadata <- read.csv("/Users/savellke/Documents/Analysis/mm10_exp1/March2023/metadataPosHC.csv", row.names = 1)

#check that the metadata is set up correctly
all(rownames(metadata) == colnames(counts0))  

```
## Run DESeq2
```{r message=FALSE}
# By design, the first group listed is the "experimental" condition. 
contrast <- c("group", "positive", "homecage")
...
save(coexp, file = "~/Output/coexpPosHC.RData")

write.csv(coexp, '~/Output/coexpPosHC_04212023.csv')
...
#backgroundgenes <- data.frame(coexp$gene)

#write.csv(backgroundgenes, '/Users/savellke/Output/backgroundgenes.csv')
```
...
df$upglut[(df$up3 > 0 & df$up0 > 0 & df$up1 > 0 & df$up2 > 0)] <- 1
...
distinct <- UpSet(cm,
      ...
      set_order = c("up3","up1","up0","up2","up0g","up1g"))
...
distinctd <- UpSet(cmd,
      top_annotation = upset_top_annotation(cmd, add_numbers = TRUE),
      right_annotation = upset_right_annotation(cmd, add_numbers = TRUE),
      set_order = c("dn3","dn1","dn0","dn2","dn0g","dn1g"))

intersectd <- UpSet(cmd.,
      top_annotation = upset_top_annotation(cmd., add_numbers = TRUE),
      right_annotation = upset_right_annotation(cmd., add_numbers = TRUE),
      set_order = c("dn3","dn1","dn0","dn2","dn0g","dn1g"))
...
```{r}
pdf(file = "/Users/savellke/Output/UpSetDN.pdf",
    width = 4,
    height = 4)
#UpSet(t(cm))
UpSet(t(cmd),
      set_order = c("dn1","dn0","dn2","dn3","dn0g")
      )
dev.off()
```
##Workspace for F3 UMAPs to try to keep cluster colors and change shapes of negative and positive
It's not working 
```{r}
a <- DimPlot(glut.subset2, shape.by = "group", raster = FALSE)
b <- a + plot_annotation()


# Define a mapping of group names to shape symbols
shape_mapping <- c("negative" = 21, "positive" = 21)
fill_mapping <- c("negative" = "white", "positive" = "black")
size_mapping <- c("negative" = 0.1, "positive" = 1)

# Create a DimPlot with custom shapes
DimPlot(glut.subset2, 
        ...
        pt.size = p,
        shuffle = TRUE
        ) + scale_shape_manual(values = shape_mapping) + scale_shape_discrete(solid=T)
```

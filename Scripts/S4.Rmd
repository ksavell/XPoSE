...
```{r message=FALSE}
#Determine how many nuclei are in each cluster/group combo

#split into clusters, add the other clusters back if they have enough nuclei
#Glut
...
glut0 <- subset(glut0, idents = c("negative","homecage"))
glut1 <- subset(glut1, idents = c("negative","homecage"))
glut2 <- subset(glut2, idents = c("negative","homecage"))
glut3 <- subset(glut3, idents = c("negative","homecage"))
glut4 <- subset(glut4, idents = c("negative","homecage"))
glut5 <- subset(glut5, idents = c("negative","homecage"))


#Collapse into 1 cluster to run 'all glut' together
all.glut <- FindClusters(glut.subset2, resolution = 0)
Idents(all.glut) <- 'group'
all.glut <- subset(all.glut, idents = c("negative","homecage"))


#GABA
gaba0 <- subset(gaba.subset2, idents = "0")
gaba1 <- subset(gaba.subset2, idents = "1")
gaba2 <- subset(gaba.subset2, idents = "2")
gaba3 <- subset(gaba.subset2, idents = "3")
gaba4 <- subset(gaba.subset2, idents = "4")
gaba5 <- subset(gaba.subset2, idents = "5")
gaba6 <- subset(gaba.subset2, idents = "6")
...
#Subset homecage and negative
gaba0 <- subset(gaba0, idents = c("negative","homecage"))
gaba1 <- subset(gaba1, idents = c("negative","homecage"))
gaba2 <- subset(gaba2, idents = c("negative","homecage"))
gaba3 <- subset(gaba3, idents = c("negative","homecage"))
gaba4 <- subset(gaba4, idents = c("negative","homecage"))
gaba5 <- subset(gaba5, idents = c("negative","homecage"))
gaba6 <- subset(gaba6, idents = c("negative","homecage"))
...
all.gaba <- subset(all.gaba, idents = c("negative","homecage"))
...
#subset the cluster to test, here they are combined into one called "0"
counts0g <- pb0g[["2"]] ##change this too
...
#subset the cluster to test, here they are combined into one called "0"
counts0g <- pb0g[["3"]] ##change this too
...
#subset the cluster to test, here they are combined into one called "0"
counts0g <- pb0g[["5"]] ##change this too
...
#subset the cluster to test, here they are combined into one called "0"
counts0g <- pb0g[["6"]] ##change this too



#generate metadata table ##FIX THIS
metadata <- read.csv("/Users/woodskad/Library/CloudStorage/Box-Box/mRFP-snSeq/Project1_XPoSEseq/XPoSEseq_manuscript/S4/metadataC.csv", row.names = 1)

#check that the metadata is set up correctly
all(rownames(metadataC) == colnames(countsa))  

```
## Run DESeq2
```{r message=FALSE}
# By design, the first group listed is the "experimental" condition. 
contrast <- c("group", "negative", "homecage")
...
ddsa <- ddsa[ rowSums(counts(ddsa)) > 5, ] #accurate gene count for multiple comparisons
...
#Cluster 4
dds4 <- DESeqDataSetFromMatrix(counts4, 
                              colData = metadata, 
                              design = ~ group)

dds4 <- dds4[ rowSums(counts(dds4)) > 5, ]

dds4 <- DESeq(dds4)

res4 <- results(dds4, ##change here
               contrast = contrast,
               alpha = 0.05,
               pAdjustMethod = "fdr")

res_tbl4 <- res4 %>% ##change here
        data.frame() %>%
        rownames_to_column(var="gene") %>%
        as_tibble()

#Cluster 5
dds5 <- DESeqDataSetFromMatrix(counts5, 
                              colData = metadata, 
                              design = ~ group)

dds5 <- dds5[ rowSums(counts(dds5)) > 5, ]

dds5 <- DESeq(dds5)

res5 <- results(dds5, ##change here
               contrast = contrast,
               alpha = 0.05,
               pAdjustMethod = "fdr")

res_tbl5 <- res5 %>% ##change here
        data.frame() %>%
        rownames_to_column(var="gene") %>%
        as_tibble()
...
#Cluster 2
dds2g <- DESeqDataSetFromMatrix(counts2g, 
                              colData = metadata, 
                              design = ~ group)

dds2g <- dds2g[ rowSums(counts(dds2g)) > 5, ]

dds2g <- DESeq(dds2g)

res2g <- results(dds2g, ##change here
               contrast = contrast,
               alpha = 0.05,
               pAdjustMethod = "fdr")

res_tbl2g <- res2g %>% ##change here
        data.frame() %>%
        rownames_to_column(var="gene") %>%
        as_tibble()

#Cluster 3
dds3g <- DESeqDataSetFromMatrix(counts1g, 
                              colData = metadata, 
                              design = ~ group)

dds3g <- dds3g[ rowSums(counts(dds3g)) > 5, ]

dds3g <- DESeq(dds3g)

res3g <- results(dds3g, ##change here
               contrast = contrast,
               alpha = 0.05,
               pAdjustMethod = "fdr")

res_tbl3g <- res3g %>% ##change here
        data.frame() %>%
        rownames_to_column(var="gene") %>%
        as_tibble()
...
#Cluster 5
dds5g <- DESeqDataSetFromMatrix(counts5g, 
                              colData = metadata, 
                              design = ~ group)

dds5g <- dds5g[ rowSums(counts(dds5g)) > 5, ]

dds5g <- DESeq(dds5g)

res5g <- results(dds5g, ##change here
               contrast = contrast,
               alpha = 0.05,
               pAdjustMethod = "fdr")

res_tbl5g <- res5g %>% ##change here
        data.frame() %>%
        rownames_to_column(var="gene") %>%
        as_tibble()

#Cluster 6
dds6g <- DESeqDataSetFromMatrix(counts6g, 
                              colData = metadata, 
                              design = ~ group)

dds6g <- dds6g[ rowSums(counts(dds6g)) > 5, ]

dds6g <- DESeq(dds6g)

res6g <- results(dds6g, ##change here
               contrast = contrast,
               alpha = 0.05,
               pAdjustMethod = "fdr")

res_tbl6g <- res6g %>% ##change here
        data.frame() %>%
        rownames_to_column(var="gene") %>%
        as_tibble()
```
